...
class EventMap extends Component<{}>
{
    static navigationOptions = ({navigation})=>{
        return {
            header: <GradientHeader title={"Bản đồ sự kiện"} navigation={navigation} />
        }
    };
    ...
    componentWillReceiveProps(nextProps){
        if (nextProps.listEvent.length !== 0){
            let first_hotel = nextProps.listEvent[0];
...
export default connect(mapStateToProps, mapActionToProps)(EventMap);
